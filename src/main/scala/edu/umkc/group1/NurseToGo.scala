 ...
 * Modified by Jordan Larson on 10/26/2015
 */
...
object NurseToGo {
  ...
  def extractDescriptors(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    ...
    featuresSeq.saveAsTextFile(PathSettings.FEATURES_PATH)
    println("Total size : " + data.size)
  }

  def LDAGrouping(sc: SparkContext): Unit = {
    // Load and parse the data
    val data = sc.textFile(PathSettings.FEATURES_PATH)
  ...
  def createHistogram(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    ...
    val sameModel = KMeansModel.load(sc, PathSettings.KMEANS_PATH)
  ...
  def generateNaiveBayesModel(sc: SparkContext): Unit = {
    if (Files.exists(Paths.get(PathSettings.NAIVE_BAYES_PATH))) {
      println(s"${PathSettings.NAIVE_BAYES_PATH} exists, skipping Naive Bayes model formation..")
      return
    }
    ...
    // Save and load model
    model.save(sc, PathSettings.NAIVE_BAYES_PATH)
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    val path = "files/Train/ant/image_0002.jpg"
    ...
    val nbModel = NaiveBayesModel.load(sc, PathSettings.NAIVE_BAYES_PATH)
  ...
  def main(args: Array[String]) {
    ...
    val images = sc.wholeTextFiles(s"${PathSettings.INPUT_DIR}/*/*.jpg").cache()
